...
- `main` : branche de préparation de release et de référence de production.

Cycle attendu :

1. Chaque push sur `dev` exécute la CI et met à jour la pre-release `dev-latest`.
2. Chaque push sur `main` exécute le pipeline technique de release, crée le tag `vX.Y.Z`, publie une release GitHub en brouillon, puis prépare la branche `dev` pour la prochaine version alpha.
...
| **Release Technical Pipeline** (`release.yml`) | Push `main` + manuel | Pipeline de release technique : normalisation version, analyse statique Semgrep, analyse statique Flutter (annotations), quality gate, tests, artefacts (Windows + PDF docs + SBOM), scan Trivy, tag, release draft, sync des branches |
...
Automatiser le cycle technique de release à partir de `main` :

1. Calculer et normaliser la version stable depuis `belotable/pubspec.yaml`.
2. Exécuter les scans Semgrep et appliquer le quality gate.
...
- Met à jour `belotable/pubspec.yaml` sur la branche `main` vers la version stable
...
- Les branches `main` et `dev` doivent exister.
